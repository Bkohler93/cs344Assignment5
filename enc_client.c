...
void initialMessage(int socketFD, char* port) {
  ...
  char res;
  int numSend = send(socketFD, "e", 1, 0); 
...
void receiveEncryptedText(char textChunks[NUM_CHUNKS][READ_BUFF], int socketFD) {
...
void returnEncryptedText(char textChunks[NUM_CHUNKS][READ_BUFF]) {
...
int main(int argc, char *argv[]) {
  ...
  * Store number of characters/chunks from each file in 2d int array.
  *   - this 2d array must be unpaked into other variables immediately
  * after function is called because it is stored in static var
  */
  char plainTextChunks[NUM_CHUNKS][READ_BUFF];
  int *numsPlain = fillTextChunks(plainTextChunks, argv[1]); 
  int numCharsPlain = numsPlain[0];
  int numChunksPlain = numsPlain[1];
  ...
  if (numCharsKey < numCharsPlain == 1) {
  ...
  //send plain text
  sendText(plainTextChunks, socketFD, numChunksPlain);

  //send key text
  sendText(keyTextChunks, socketFD, numChunksKey);

  //receive encrypted text, store into plainTextFile buffer
  receiveEncryptedText(plainTextChunks, socketFD); 

  //write encrypted text to stdout
  returnEncryptedText(plainTextChunks);
